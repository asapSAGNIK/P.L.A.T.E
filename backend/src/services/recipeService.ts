...
export async function saveRecipeForUser(
  ...
  try {
    ...
    if (!recipeData.title || !recipeData.source) {
      logger.error('Missing required fields', { recipeData });
    ...
    if (recipeData.original_recipe_id) {
      ...
      if (findError && findError.code !== 'PGRST116') {
        logger.error('Error finding existing recipe', { 
          error: findError, 
          originalRecipeId: recipeData.original_recipe_id 
        });
      ...
      if (existingRecipe) {
        ...
        logger.info('Using existing recipe', { recipeId, originalRecipeId: recipeData.original_recipe_id });
      } else {
        // Create new recipe
        ...
        logger.info('Created new recipe', { recipeId, originalRecipeId: recipeData.original_recipe_id });
      }
    } else {
      ...
      recipeId = newRecipe.id;
      logger.info('Created new recipe', { recipeId });
    }
    ...
    if (checkError) {
      logger.error('Error checking existing saved recipe', { error: checkError });
      return { success: false, error: 'Failed to check existing saved recipe' };
    }

    if (existingSaved) {
      logger.info('Recipe already saved by user', { userId, recipeId });
    ...
    if (saveError) {
      ...
      if (saveError.code === '23505') {
        logger.info('Recipe already saved by user (unique constraint)', { userId, recipeId });
        return { success: true, recipeId };
      }
      logger.error('Error saving recipe for user', { error: saveError });
      return { success: false, error: 'Failed to save recipe' };
    }

    logger.info('Successfully saved recipe for user', { userId, recipeId, savedRecipeId: savedRecipe.id });
    return { success: true, recipeId };

  } catch (error) {
    logger.error('Unexpected error saving recipe', { error, userId });
 ...
 */
export async function getUserSavedRecipes(userId: string): Promise<{ success: boolean; recipes?: SavedRecipe[]; error?: string }> {
  try {
    logger.info('Fetching saved recipes for user', { userId });
    ...
    if (error) {
      logger.error('Error fetching saved recipes', { error, userId });
...
export async function removeSavedRecipe(
  ...
  try {
    logger.info('Removing saved recipe for user', { userId, recipeId });
    ...
    if (error) {
      logger.error('Error removing saved recipe', { error, userId, recipeId });
      return { success: false, error: 'Failed to remove saved recipe' };
    }

    logger.info('Successfully removed saved recipe', { userId, recipeId });
    return { success: true };

  } catch (error) {
    logger.error('Unexpected error removing saved recipe', { error, userId, recipeId });
    return { success: false, error: 'Unexpected error occurred' };
  }
}
...
export async function checkIfRecipeSaved(
  ...
  try {
    ...
    if (error && error.code !== 'PGRST116') {
      logger.error('Error checking if recipe is saved', { error, userId, recipeId });
    ...
    logger.error('Unexpected error checking recipe status', { error, userId, recipeId });
    return { success: false, error: 'Unexpected error occurred' };
  }
}
 ...
 */
export function convertAIRecipeToSaveRequest(aiRecipe: AIRecipe): SaveRecipeRequest {
  return {
