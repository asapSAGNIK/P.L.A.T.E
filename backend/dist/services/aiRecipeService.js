...
exports.generateAIRecipes = generateAIRecipes;
 ...
 */
function generateRecipePrompt(ingredients, query, filters, recipeNumber, mode = 'fridge' // Added mode parameter
) {
 ...
 */
function parseAIRecipe(aiText, recipeNumber, ingredients, filters) {
    try {
        ...
        if (!titleMatch || !ingredientsMatch || !instructionsMatch) {
            logger_1.default.warn('Failed to parse recipe components from AI response', { recipeNumber });
            return null;
        }
        ...
        ingredientsList = ingredientsList.filter(ingredient => {
            ...
            return allowedIngredients.some(allowed => ingredientLower.includes(allowed)) ||
                ...
                (ingredients && ingredients.some(userIngredient => ingredientLower.includes(userIngredient.toLowerCase())));
        });
        ...
        logger_1.default.error('Error parsing AI recipe', { error, recipeNumber });
        return null;
    }
}
/**
 ...
 */
async function generateSingleRecipe(ingredients, query, filters, recipeNumber, mode = 'fridge' // Default to fridge mode
) {
    try {
        const recipePrompt = generateRecipePrompt(ingredients, query, filters, recipeNumber, mode);
        logger_1.default.info('Generating AI recipe', { recipeNumber, ingredients, filters, mode });
        const response = await axios_1.default.post('https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent', {
        ...
        if (!aiText) {
            logger_1.default.warn('No AI text generated', { recipeNumber });
            return null;
        }
        const parsedRecipe = parseAIRecipe(aiText, recipeNumber, ingredients, filters);
        if (parsedRecipe) {
            logger_1.default.info('Successfully generated AI recipe', {
                recipeNumber,
        ...
        logger_1.default.error('Error generating single recipe', {
            error: error.message,
        ...
        if (error.response?.status === 503) {
            logger_1.default.warn('Gemini API temporarily unavailable (503)', { recipeNumber });
        }
        else if (error.response?.status === 429) {
            logger_1.default.warn('Gemini API rate limit exceeded (429)', { recipeNumber });
        }
        else if (error.response?.status === 400) {
            logger_1.default.warn('Gemini API bad request (400)', { recipeNumber, error: error.response.data });
        }
        return null;
    }
}
 ...
 */
async function generateAIRecipes(request) {
    ...
    logger_1.default.info('Starting AI recipe generation', { ingredients, query, filters, mode });
        ...
        try {
            ...
            if (recipe) {
                ...
                const isDuplicate = aiRecipes.some(existingRecipe => {
                    ...
                    const hasSimilarWords = similarWords.some(word => existingRecipe.title.toLowerCase().includes(word) && recipe.title.toLowerCase().includes(word));
                ...
                if (!isDuplicate) {
                    ...
                    logger_1.default.warn('Duplicate or similar recipe detected, skipping', {
                        title: recipe.title,
                    ...
                    if (alternativeRecipe && !aiRecipes.some(r => {
                        ...
                        const similarWords = ['cheesy', 'cheese', 'simple', 'easy', 'quick'];
                        const hasSimilarWords = similarWords.some(word => r.title.toLowerCase().includes(word) && alternativeRecipe.title.toLowerCase().includes(word));
                        return titleMatch || hasSimilarWords;
                    })) {
            ...
            logger_1.default.error(`Error generating recipe ${i + 1}`, { error });
            // If we hit rate limit, try to generate at least one recipe
            if (error.response?.status === 429 && aiRecipes.length === 0) {
                logger_1.default.warn('Rate limit hit, trying one more time with longer delay');
                ...
                try {
                    ...
                    logger_1.default.error('Fallback recipe generation also failed', { fallbackError });
                }
            }
...
//# sourceMappingURL=aiRecipeService.js.map